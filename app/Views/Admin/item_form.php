...
<form method="post" action="/admin/items/<?php echo (int)($item['id'] ?? 0); ?>" class="space-y-4">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <div>
      ...
      <input class="input w-full" type="number" name="ingredient_id" value="<?php echo htmlspecialchars((string)($input['ingredient_id'] ?? '')); ?>">
    </div>
    <div>
      <label class="block text-sm font-medium mb-1">Product ID <span class="text-xs text-text-muted">(optional)</span></label>
      <input class="input w-full" type="number" name="product_id" value="<?php echo htmlspecialchars((string)($input['product_id'] ?? '')); ?>">
    </div>
  </div>
  <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
    <div>
      <label class="block text-sm font-medium mb-1">Quantity</label>
      <input class="input w-full" type="text" name="quantity" value="<?php echo htmlspecialchars((string)($input['quantity'] ?? '1')); ?>">
    </div>
    <div>
      <label class="block text-sm font-medium mb-1">Unit <span class="text-xs text-text-muted">(optional)</span></label>
      <input class="input w-full" type="text" name="unit" value="<?php echo htmlspecialchars((string)($input['unit'] ?? '')); ?>">
    </div>
    <div>
      <label class="block text-sm font-medium mb-1">Purchase Date <span class="text-xs text-text-muted">(YYYY-MM-DD)</span></label>
      <input class="input w-full" type="date" name="purchase_date" value="<?php echo htmlspecialchars((string)($input['purchase_date'] ?? '')); ?>">
    </div>
  </div>
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <div>
      <label class="block text-sm font-medium mb-1">Expiration Date <span class="text-xs text-text-muted">(YYYY-MM-DD)</span></label>
      <input class="input w-full" type="date" name="expiration_date" value="<?php echo htmlspecialchars((string)($input['expiration_date'] ?? '')); ?>">
    </div>
    <div>
      <label class="block text-sm font-medium mb-1">Entered Name <span class="text-xs text-text-muted">(optional)</span></label>
      <input class="input w-full" type="text" name="entered_name" value="<?php echo htmlspecialchars((string)($input['entered_name'] ?? '')); ?>">
    </div>
  </div>
  <div>
    <label class="block text-sm font-medium mb-1">Entered Brand <span class="text-xs text-text-muted">(optional)</span></label>
    <input class="input w-full" type="text" name="entered_brand" value="<?php echo htmlspecialchars((string)($input['entered_brand'] ?? '')); ?>">
  </div>
