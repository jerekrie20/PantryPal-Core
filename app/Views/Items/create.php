    ...
    <div class="card p-6 md:p-8">
        <form action="/items" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <div role="radiogroup" aria-labelledby="api-kind-label" class="inline-flex w-full sm:w-auto bg-bg-subtle rounded-lg p-1 gap-1">
                    <?php
                    $kinds = [
                    ...
                    foreach ($kinds as $k):
                        ...
                        <label class="flex-1 cursor-pointer">
                            <input type="radio" name="api_kind" value="<?= htmlspecialchars($k['key']) ?>" class="sr-only peer" <?= $checked ? 'checked' : '' ?> />
                ...
                <div>
                    ...
                    <select id="unit" name="unit" class="w-full">
                        ...
                        <?php if ($selectedUnit && !array_key_exists($selectedUnit, $units)): ?>
                            <option value="<?= htmlspecialchars($selectedUnit) ?>" selected><?= htmlspecialchars($selectedUnit) ?> (custom)</option>
                        <?php endif; ?>
                    </select>
